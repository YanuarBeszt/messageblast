...
$(document).ready(function () {
  ...
  function info(data) {
    Swal.fire({
      ...
      window.location = `${BASE_URL}Email/draft`;
    });
  }
  function swal_alert2(data) {
    Swal.fire({
      title: "Failed",
      text: data,
      type: "error",
    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        if (res.code === 1) {
          ...
          toastr.error(
            "Failed send to " + res.pesan + ", Invalid Email Password!"
          );
        }
      },
    });
  ...
  $(".sendEmail").on("click", function () {
    ...
    if (email != "") {
      info("Please wait, the message is being sent.");
      ...
      swal_alert2("Form cannot be empty!");
    }
    // setTimeout(function () {
